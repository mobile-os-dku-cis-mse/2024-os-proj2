...
typedef struct {
    ...
    int swapped_out;     // 스왑 아웃 횟수
    int swapped_in;      // 스왑 인 횟수 (새로 추가)
} process;
...
// FIFO 알고리즘
int fifo_queue[PAGES];  // FIFO 큐: 물리 메모리 프레임 번호를 저장
int fifo_front = 0;     // 큐의 시작
int fifo_rear = 0;      // 큐의 끝
int fifo_count = 0;     // 큐에 있는 페이지 수
...
void initialize_system() {
    ...
    fifo_front = fifo_rear = fifo_count = 0;  // FIFO 큐 초기화
...
int allocate_free_page() {
    if (free_page_count > 0) {
        int free_page = free_page_list[--free_page_count];
        enqueue_fifo(free_page);  // FIFO 큐에 새로 할당된 페이지 추가
        return free_page;
    } else {
        // 메모리가 부족한 경우 FIFO 큐에서 가장 오래된 페이지를 제거
        if (fifo_count > 0) {
            return dequeue_fifo();
        } else {
            fprintf(stderr, "FIFO 큐가 비어있습니다. 할당 불가능!\n");
            exit(EXIT_FAILURE);
        }
    }
}
...
void enqueue_fifo(int frame_number) {
    for (int i = 0; i < fifo_count; i++) {
        ...
        if (fifo_queue[index] == frame_number) {
            fprintf(simulation_log, "FIFO 큐: 중복된 페이지 %d 추가 시도\n", frame_number);
    ...
    if (fifo_count < PAGES) {
        ...
        fifo_count++;
        fprintf(simulation_log, "FIFO 큐: 페이지 %d 추가 (현재 큐 크기: %d)\n", frame_number, fifo_count);
    } else {
        fprintf(stderr, "FIFO 큐 오버플로우 발생! 더 이상 추가할 수 없습니다.\n");
        exit(EXIT_FAILURE);
    }
}
...
int dequeue_fifo() {
    if (fifo_count > 0) {
        ...
        fifo_count--;
        fprintf(simulation_log, "FIFO 큐: 페이지 %d 제거 (현재 큐 크기: %d)\n", frame_number, fifo_count);
        return frame_number;
    } else {
        fprintf(stderr, "FIFO 큐 언더플로우 발생! 더 이상 제거할 수 없습니다.\n");
        exit(EXIT_FAILURE);
    }
}
...
void handle_page_fault(int process_id, int virtual_address, int tick, int is_write) {
    ...
    if (!table) {
        ...
        fprintf(simulation_log, "Tick: %d, 프로세스 %d: 새로운 페이지 테이블 생성 (디렉터리 인덱스 %d)\n",
                tick, process_id + 1, dir_index);
    }

    // 페이지가 이미 유효하다면, 아무 작업도 하지 않음
    ...
    // 메모리가 부족한 경우 FIFO를 통해 페이지 교체
    if (free_page == -1) {
        int victim_frame = dequeue_fifo();
        int swapped_out = 0;

        // victim_frame을 사용하는 프로세스와 페이지를 찾아 스왑 아웃
        for (int i = 0; i < NUM_PROCESSES; i++) {
            for (int d = 0; d < PAGE_DIRECTORY_ENTRIES; d++) {
                ...
                if (dir) {
                    for (int t = 0; t < PAGE_TABLE_ENTRIES; t++) {
                        if (dir[t].valid && dir[t].frame_number == victim_frame) {
                            fprintf(swapping_log, "Tick: %d, 프로세스 %d의 페이지 [%d, %d] (VA: 0x%08X, Frame: %d) 스왑아웃\n",
                                    tick, processes[i].pid, d, t, (d << 20) | (t << 12), victim_frame);
                            dir[t].valid = 0;  // 페이지 무효화
                            processes[i].swapped_out++;
                            swap_count++;
                            disk_io_count++;  // 스왑 아웃 시 디스크 I/O 증가
                            swapped_out = 1;
                            break;
                        }
                    }
        ...
        if (!swapped_out) {
            fprintf(stderr, "스왑 아웃 실패: victim_frame %d을 찾을 수 없습니다.\n", victim_frame);
            exit(EXIT_FAILURE);
        }
        free_page = victim_frame;
    }
    ...
    proc->page_faults++;
    proc->swapped_in++;  // 프로세스별 스왑 인 횟수 증가
    swap_in_count++;
    disk_io_count++;  // 스왑 인 시 디스크 I/O 증가
...
int calculate_physical_address(int frame_number, int offset) {
    if (frame_number < 0 || offset >= PAGE_SIZE) {
        fprintf(stderr, "물리 주소 계산 오류: frame_number=%d, offset=%d\n", frame_number, offset);
        return -1;
    }
...
void print_simulation_summary(int tick) {
    ...
    for (int i = 0; i < NUM_PROCESSES; i++) {
        process *proc = &processes[i];
        printf("프로세스 %d: 페이지 폴트: %d, 메모리 접근: %d, 스왑 아웃: %d, 스왑 인: %d\n",
               proc->pid, proc->page_faults, proc->memory_accesses, proc->swapped_out, proc->swapped_in);
    }
